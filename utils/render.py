...
def visualizeReconstruction(img, img_size, gt_kp, vertices, pred_kp, camera, renderer, color='pink', focal_length=1000):
    """
...
def visualizeReconstructionTest(img, img_size, gt_kp, vertices, pred_kp, camera, renderer, score, color='pink', focal_length=1000):
    """
    Overlays gt_kp and pred_kp on img.
    Draws vert with text.
    Renderer is an instance of SMPLRenderer.
...
def visualizeReconstructionAndAtt(img, img_size, vertices_full, vertices_2d, camera, renderer, ref_points, attention, focal_length=1000):
    """
    Overlays gt_kp and pred_kp on img.
    Draws vert with text.
    ...
    for ii in range(len(selected_joints)):
        ...
        for item in total_to_draw:
            ...
            ref_point = item[1]
            vertex = item[2]
            plotOneLine(ref_point, vertex, image, ii, alpha=(attention_score-thres)/(max_att_score-thres)  )
        # code.interact(local=locals())
        if len(combined)==0:
            combined = image
        else:
...
def visualizeReconstructionAndAttLocal(img, img_size, vertices_full, vertices_2d, camera, renderer, ref_points, attention, color='light_blue', focal_length=1000):
    """
    Overlays gt_kp and pred_kp on img.
    Draws vert with text.
    Renderer is an instance of Renderer.
...
def visualizeReconstructionWOText(img, img_size, vertices, camera, renderer, color='pink', focal_length=1000):
    """
    Overlays gt_kp and pred_kp on img.
...
def plotOneLine(ref, vertex, img, color_index, alpha=0.0, line_thickness=None):
    # 13,6,7,8,3,4,5
...
def visualizeMesh( renderer,
    ...
    for i in range(min(batch_size, 10)):
        ...
        rend_img = visualizeReconstruction(img, 224, gt_keypoints_2d_, vertices, pred_keypoints_2d_, cam, renderer)
        rend_img = rend_img.transpose(2,0,1)
        rend_imgs.append(torch.from_numpy(rend_img))   
    rend_imgs = makeGrid(rend_imgs, nrow=1)
    return rend_imgs

def visualizeMeshTest( renderer,
                    images,
                    gt_keypoints_2d,
                    pred_vertices, 
                    pred_camera,
    ...
    for i in range(min(batch_size, 10)):
        ...
        score = PAmPJPE[i]
        # Visualize reconstruction and detected pose
        rend_img = visualizeReconstructionTest(img, 224, gt_keypoints_2d_, vertices, pred_keypoints_2d_, cam, renderer, score)
        rend_img = rend_img.transpose(2,0,1)
        rend_imgs.append(torch.from_numpy(rend_img))   
    rend_imgs = makeGrid(rend_imgs, nrow=1)
    return rend_imgs

def visualizeMeshWOText( renderer,
                    images,
                    pred_vertices, 
                    pred_camera):
    """Tensorboard logging."""
    ...
    for i in range(min(batch_size, 1)):
        ...
        rend_img = visualizeReconstructionWOText(img, 224, vertices, cam, renderer, color='hand')
        rend_img = rend_img.transpose(2,0,1)
        rend_imgs.append(torch.from_numpy(rend_img))   
    rend_imgs = makeGrid(rend_imgs, nrow=1)
    return rend_imgs
