...
from .hand_mesh_tsv import HandMeshTSVYamlDataset
...
def build_hand_dataset(yaml_file, args, is_train=True, scale_factor=1):
    ...
    if not op.isfile(yaml_file):
        yaml_file = op.join(args.dataset_folder, yaml_file)
...
def make_hand_data_loader(args, yaml_file, is_train=True, start_iter=0, scale_factor=1):
    ...
    if is_train==True:
        ...
        args.logger.createLog("DATA_LOADER", "Train with {} images per batch.".format(args.batch_size))
        args.logger.createLog("DATA_LOADER", "Total training steps {}".format(num_iters))
    else:
        shuffle = False
        num_iters = None
        start_iter = 0
