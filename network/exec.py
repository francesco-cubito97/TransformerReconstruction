...
def run(args, train_dataloader, TransRecon_model, mano_model, renderer, mesh_sampler):
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        mpm_mask_ = mpm_mask.expand(-1, -1, 577)
        mbm_mask_ = mbm_mask.expand(-1, -1, 577)
        meta_masks = torch.cat([mpm_mask_, mbm_mask_], dim=1)
        
        # Forward-pass
        pred_camera, pred_3d_joints, pred_vertices_sub, pred_vertices, pred_pose, pred_betas = TransRecon_model(images, mano_model, mesh_sampler, meta_masks=meta_masks, is_train=True)
